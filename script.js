(function(){
  ...
  const rulesUK = [
    ...
    { cat:"Запит конфіденційних даних", level:"high",
      test:(t)=> /(cvv|пароль|пін-?код|код з смс|підтвердіть картку|введіть.*(картк|дан))/i.test(t),
    ...
    { cat:"Заміна SIM / перехоплення номера", level:"high",
      test:(t)=> /(перевипуск|заміна).{0,15}(сім|sim)/i.test(t) || /(продиктуйте|назвіть|скажіть).{0,20}код/i.test(t) || /підтвердіть номер телефону/i.test(t),
    ...
    { cat:"Фальшива виплата/компенсація", level:"med",
      test:(t)=> /(виплат|компенсац|субсиді|грант|допомог[аи])/i.test(t) && /(отрима|оформ|призначен|нарахован)/i.test(t),
    ...
    { cat:"Психологічний тиск / терміновість", level:"med",
      test:(t)=> /(терміново|негайно|протягом \d+ годин|щоб уникнути блокування|остання можливість)/i.test(t),
    ...
    { cat:"Прохання від «знайомого»", level:"med",
      test:(t)=> /(допоможи|позич).{0,60}(гроші|кошти|грн)/i.test(t) || /переказ.{0,20}(терміново|негайно)/i.test(t) || /(терміново|негайно).{0,20}переказ/i.test(t),
