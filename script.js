(function(){
  ...
  const rulesUK = [
    ...
    { cat:"Заміна SIM / перехоплення номера", level:"high",
      test:(t)=> /(перевипуск.*сім|заміна.*сім|новою sim|код для оператора|підтвердіть номер телефону)/i.test(t),
    ...
    { cat:"Фальшива виплата/компенсація", level:"med",
      test:(t)=> /(виплат|компенсац|субсиді|грант|допомог[аи]).{0,30}(отрима|оформ|призначен)/i.test(t),
    ...
    { cat:"Прохання від «знайомого»", level:"med",
      test:(t)=> /(допоможи|позич|переказ.{0,15}терміново).{0,25}(гроші|кошти|грн)/i.test(t),
